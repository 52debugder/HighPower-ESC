...
void OPA_Init(enumOPA opa, OPA_InitTypeDef *OPA_InitStruct)
{
    ...
    if (opa == OPA0)
    {
        if (OPA_InitStruct->OPA_Gain == PGA_GAIN_32)
        {
            maxmagflg = 1;
        }
    ...
    if (maxmagflg == 1)
    {
        OPA_InitStruct->OPA_IT = PGA_IT_1P5;
    }
    else
    {
        OPA_InitStruct->OPA_IT = PGA_IT_1;
    }
...
void OPA_OUT(enumOPA opa, FuncState state)
{
    ...
    if (state != DISABLE)
    {
        if (opa == OPA0)
        {
            REG_WRITE_BIT_POS_MASK(SYS_AFE_REG2, SYS_AFE_REG2_OPAOUT_EN, 1);
        }
        else if (opa == OPA1)
        {
            REG_WRITE_BIT_POS_MASK(SYS_AFE_REG2, SYS_AFE_REG2_OPAOUT_EN, 2);
        }
        else if (opa == OPA2)
        {
            REG_WRITE_BIT_POS_MASK(SYS_AFE_REG2, SYS_AFE_REG2_OPAOUT_EN, 3);
        }
        else if (opa == OPA3)
        {
            REG_WRITE_BIT_POS_MASK(SYS_AFE_REG2, SYS_AFE_REG2_OPAOUT_EN, 4);
        }
    }
    else
    {
        REG_WRITE_BIT_POS_MASK(SYS_AFE_REG2, SYS_AFE_REG2_OPAOUT_EN, 0);
    }
    SYS_WR_PROTECT = 0x0; /* 开启系统寄存器写保护 */
}
